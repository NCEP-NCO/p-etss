...
if [[ "$area" = 'con' ]]; then
   etsurge_grib=${COMIN}/petss.t${cyc}z.stormtide_${propf}_1hr_inc_${datum}.${area}2p5km.grib2
   box=(-130.00 15.0 -63.00 53)
elif [[ "$area" = 'ala' ]]; then
   etsurge_grib=${COMIN}/petss.t${cyc}z.stormtide_${propf}_1hr_inc_${datum}.${area}3km.grib2
...
for i in `seq 1 102`; do
  ...
  shpfile=${SHPpetss}/${area}_${datumm}_${cyc}_1hr_msg_${i}_${prop}.shp
  startmsg
