...
C
C MAIN PROGRAM: MDL_CY_PUV10
C   PRGMMR: ALLEN            ORG: OSD211      DATE: 2000-03-14
C                THEN APPLY 35 DEGREE WINDOW FOR P-U-V
C
C ABSTRACT: EXTRACT SURFACE P,U, AND V FROM AVN GRIB FILES THEN
C           MAKE 35 DEGREE WINDOWED DATA SET. 
C           THE TEMPORARY OUTPUT FILES ARE FORTRAN ARRAYS 
C           (INSTEAD OF GRIB SUBSET 'GRBXX') AND ARE NOT SAVED.  
C
C PROGRAM HISTORY LOG:
C   94-10-25  KIM/CHEN
C   95-11-25  C.MO    COMBINE OLD GRIB_PUV.F AND GRIB_EX.F
C                     BY THE USE OF W3LIB SUBROUTINE GETGB.
C   98-07-28  J.CHEN  MODIFY FOR YEAR 2000 COMPLIANCE. 10-M WIND. 
C   98-09-21  SHIREY  CALLS TO W3TAG ORIGINALLY COMMENTED OUT...
C                     MADE THE LINES EXECUTABLE IN THE CODE.
C   00-03     CHEN    CONVERTED TO RUN ON THE IBM SP.  CHANGES WERE
C                     MADE IN OPENING GRIB FILES. CHANGED FROM .9995
C                     SIGMA WINDS TO 10 M WINDS
C   04-15     H.Liu   CONVERTED TO USE GRIB2 FORMAT GFS FILES (.5 by .5 deg)
C!
C!  01-16     Huiqing Liu /MDL to process 102 hrs wind fields
...
!-----------------------------------------------------------
      subroutine wind_fcst_gefs(res_wnd)
!     fortran 90 
!
      use grib_mod
