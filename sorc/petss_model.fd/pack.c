...
/*  output prev/run */
      if (run != 0) {
         run = run - 1;
         memBitWrite (&run, sizeof (uInt4), *ptr, 8, bufLoc, &numUsed);
         *ptr = *ptr + numUsed;
 ...
 *   > for speed sake, may want entire array passed in?
 ****************************************************************************/
int UnStuff_xxx (FILE * fp, uChar * gbuf, sChar * gbufLoc, sInt4 * val,
                 char f_flag)
{
   static sInt4 prev = 0; /* prev = [-150..360] or 999 */
   static uInt4 run = 0;
   uInt4 temp;
   ...
   if (f_flag == 1) {
      /* Clear the get byte. */
/*    tFlush (0, tp);*/
      /* Flush the get buffer. */
      fileBitRead (&run, sizeof (uInt4), 0, fp, gbuf, gbufLoc);
      run = 0;
      *val = 0;
      return 0;
   }
...
/*    tGetBit (&run, sizeof (run), 8, tp);*/
      fileBitRead (&run, sizeof (uInt4), 8, fp, gbuf, gbufLoc);
      run += 1;
/*    tGetBit (&temp, sizeof (temp), 9, tp);*/
      fileBitRead (&temp, sizeof (uInt4), 9, fp, gbuf, gbufLoc);
      prev = temp - 150;
      if (prev == 361)
         prev = 999;
      *val = prev;
 ...
 *   > for speed sake, may want entire array passed in?
 ****************************************************************************/
int UnStuff2_xxx (FILE * fp, uChar * gbuf, sChar * gbufLoc, sInt4 * val,
                  char f_flag)
{
   static sInt4 prev = 0; /* prev = [-320..700] or 999 */
   static uInt4 run = 0;
   uInt4 temp;
...
/*    tGetBit (&temp, sizeof (temp), 10, tp);*/
      fileBitRead (&temp, sizeof (uInt4), 10, fp, gbuf, gbufLoc);
      prev = temp - 320;
      if (prev == 701)
         prev = 999;
      *val = prev;
...
uInt4 ltln_val[32] = { 0, 1, 2, 4, 8, 12, 16, 24,
...
{
   uChar lit, index, lit2, first, second;
   uInt4 enc_x;
   static uChar last = 0, len = 0;

   if (f_flag == 0) {
/* need 4,5, or 6 bits depending on last, and len. */
      if (len == 2) {
         fileBitRead (&lit2, sizeof (uChar), 4, fp, gbuf, gbufLoc);
/*      tGetBit (&lit2, sizeof (lit2), 4, tp);*/
         lit = (unsigned char) (((last & 3) << 4) + lit2);
      } else if (len == 1) {
         fileBitRead (&lit2, sizeof (uChar), 5, fp, gbuf, gbufLoc);
/*      tGetBit (&lit2, sizeof (lit2), 5, tp);*/
         lit = (unsigned char) (((last & 1) << 5) + lit2);
      } else {
         fileBitRead (&lit, sizeof (uChar), 6, fp, gbuf, gbufLoc);
         ...
         *x = (*sgn) * ((lit & 28) >> 2);
         ...
         if (lit >= 8) {
            *x = (*sgn) * ((lit >> 1) + 4);
            ...
            len = 0;
            fileBitRead (&lit2, sizeof (uChar), 3, fp, gbuf, gbufLoc);
/*        tGetBit (&lit2, sizeof (lit2), 3, tp);*/
            index = (unsigned char) (((lit & 3) << 3) + lit2);
            enc_x = 0;
            if (ltln_ray[index] != 0) {
               fileBitRead (&enc_x, sizeof (uInt4), ltln_ray[index], fp,
                            gbuf, gbufLoc);
...
/* First has to == 0 (ie else if (first == 0) ) */
            ...
            if (second >= 2) {
               fileBitRead (&lit, sizeof (uChar), 1, fp, gbuf, gbufLoc);
/*          tGetBit (&lit, sizeof (lit), 1, tp);*/
               second = (unsigned char) ((second << 1) + lit);
               *x = (*sgn) * (second + 4);
               return 0;
            } else if (second == 1) {
               fileBitRead (&lit, sizeof (uChar), 2, fp, gbuf, gbufLoc);
/*          tGetBit (&lit, sizeof (lit), 2, tp);*/
               ...
               fileBitRead (&index, sizeof (uChar), 5, fp, gbuf, gbufLoc);
...
/*    tFlush (0, tp);*/
      /* Flush the get buffer. */
      fileBitRead (&index, sizeof (uChar), 0, fp, gbuf, gbufLoc);
      last = 0;
      len = 0;
      return 0;
   } else {
