...
typedef struct {
  ...
  TFLAG_ACCESS access;
  uChar f_os; /* true if file is opposite endian from operating system. */
  uChar gbuf, pbuf;
  uChar gbuf_loc, pbuf_loc; /* These are which bit from 1..8 we
           are looking at.  If 0, then gbuf is empty, but pbuf is full. */

  uChar f_stream; /* false means use fp, true means use stream */
  uChar *stream;
