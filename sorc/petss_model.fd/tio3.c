                 ...
                 TFLAG_SYSTEM sys)
{
   ...
   /* We managed to open the file, so we will attempt to set asside space
    * for the TIO data structure... */
 ...
 * Notes:
 *-------------------------------------------------------------------------*/
uChar tGet_Endian (TIO_type * tio)
{
   return (tio->f_os);
}
 ...
 * Notes:
 *-------------------------------------------------------------------------*/
int tFlush (uChar f_put, TIO_type * tio)
{
   if (f_put) {
      if (tio->pbuf_loc != 8) {
 ...
 *----------------------------------------------------------------------*/
int tSeek (TIO_type * tio, sInt4 offset, int origin)
{
                     ...
                     TIO_type * tio)
{
   ...
   uChar *dst = Dst;    /* do I need this? Yes! */
   size_t num_bytes;
   uChar dst_loc;
   int c;
   ...
   if (buf_loc >= dst_loc) {
      ...
      *(ptr++) |= (uChar) ((buf & BitRay[buf_loc]) >> (buf_loc - dst_loc));
      ...
      buf = (uChar) c;  /* buf_loc should be 8 */
      buf_loc += (uChar) (8 - dst_loc); /* 8 - (dst_loc - buf_loc) */
      /* need mask in case right shift with sign extension? should be ok
       * since buf is an unsigned char, so it fills with 0s. */
#ifdef _sysINTEL
 ...
 *-------------------------------------------------------------------------*/
uChar tReadStruct (void *Dst, size_t dst_len, const char *elem_sizes,
                   TIO_type * tio)
{
   const char *ptr1;
   uChar *dst, temp, *dst1, *dst2;
   ...
   if (tio->f_os) {
      ...
      while ((*ptr1 != '\0') && (cur_len < dst_len)) {
         if (*ptr1 == '1') {
            if (*(++ptr1) == '*') {
               /* strtoul stops at ',' or '*' or '\0' putting remainder in
                * NULL */
               cur_len +=
                  (unsigned short int) strtoul ((++ptr1), (char **) NULL,
                                                10);
               if ((ptr1 = strchr (ptr1, ',')) == NULL)
                  break; /* simpler just to break than go back to '\0'
                          * space. */
               ptr1++;
            } else {
               cur_len++;
 ...
 *-------------------------------------------------------------------------*/
uChar tPutBit (void *Src, size_t src_len, unsigned short int num_bits,
               TIO_type * tio)
{
   register uChar buf_loc, buf, *ptr;
   uChar *src = Src;
   ...
   if (buf_loc >= src_loc) {
...
#ifdef _sysINTEL
      buf |=
         (uChar) ((*(ptr--) & ((1 << src_loc) - 1)) << (buf_loc - src_loc));
#else
      buf |=
         (uChar) ((*(ptr++) & ((1 << src_loc) - 1)) << (buf_loc - src_loc));
#endif
      ...
      if (buf_loc != 0) {
         buf |=
            (uChar) ((*ptr & ((1 << src_loc) - 1)) >> (src_loc - buf_loc));
      }
   ...
   while (ptr < src + src_len) {
      ...
      if (buf_loc == 0) { /* simple case where buf and src line up.. */
         ...
         /* need mask in case right shift with sign extension? should be ok
          * since *ptr is an unsigned char so it fills with 0s. */
...
size_t tWrite (void *Src, size_t elem_size, size_t num_elem, TIO_type * tio)
{
   ...
   if (tio->f_os) {
      if (elem_size == 1) {
         ...
         src = Src;
         /* can't do n/2 swaps because we would corrupt src, which the user
          * might need. */
         ptr = src - elem_size - 1;
...
void twrite_ (int *fid, sInt4 * ptr, int *num)
{
   TIO_type *tp = tFind ((unsigned short int) *fid);
   tWrite (ptr, sizeof (sInt4), *num, tp);
}
...
void tread (int *fid, sInt4 * ptr, int *num)
{
   TIO_type *tp = tFind ((unsigned short int) *fid);
   tRead (ptr, sizeof (sInt4), *num, tp);
}
...
void twrite (int *fid, sInt4 * ptr, int *num)
{
   TIO_type *tp = tFind ((unsigned short int) *fid);
   tWrite (ptr, sizeof (sInt4), *num, tp);
}
